...
def add_seat_to_row(chart, row_index, seat_to_add)
  ...
  # at row_index in the index of the chart, then return the chart
...
def count_empty_seats(chart)
  # take a chart (e_w_i) and row (e_w_i) and return the number of empty (nil) seats in it
...
array_of_hash_objects.select{|index| index[:is_my_favorite?] == true}[0]
